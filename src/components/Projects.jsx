                ...
                {displayedProjects.map(project => (
                    <div key={project._id} className="md:w-[30%]  mb-[60px] ">
                        ...
                        {selectedProject === project._id && (
                            <div className="bg-black z-50 border border-violet-600 rounded-lg fixed md:w-[50%] w-[90%] top-[5%]  left-[5%] md:left-[25%]">
                                <div className="p-4  rounded-lg relative">
                                    <img src={project.image.url} alt={project.title} className=" w-full h-[200px] md:h-[400px] object-cover  rounded-lg" />
                                    <h2 className="text-[24px]   font-semibold mb-2 mt-5">{project.title}</h2>
                                    <p className='mt-4'><span className="text-md text-gray-400 flex gap-[15px]"> {project.techStack.map((tech,index)=>(<span className='border border-gray-400 pl-3 pr-3 pt-1 pb-1 rounded-[18px]' key={index}>{tech.trim()}</span>))}</span></p>
